...
const TaskManagerDemo = () => {
  const [tasks, setTasks] = useState([
    { id: 1, name: 'Diseñar interfaz de usuario', status: 'activo' },
    { id: 2, name: 'Implementar autenticación', status: 'activo' },
    { id: 3, name: 'Revisar código del backend', status: 'cambios' },
    { id: 4, name: 'Escribir documentación', status: 'pendiente' },
    { id: 5, name: 'Deploy a producción', status: 'entregado' }
  ]);
  ...
  // Ordenar tareas por prioridad de estado
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-2">
        <h1 className="text-xl font-bold text-white">Tareas - Demo</h1>
      </div>
      ...
      <div className="p-6 bg-slate-50 border-b border-slate-200">
        <div className="flex gap-3">
          ...
          <button
            onClick={addTask}
            className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition flex items-center gap-2 font-medium text-sm"
          >
            <Plus size={16} />
            Agregar
          </button>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-slate-100 border-b border-slate-200">
              <th className="text-left p-4 font-semibold text-slate-700 text-sm">Tarea</th>
              <th className="text-left p-4 font-semibold text-slate-700 text-sm">Estado</th>
          ...
          <tbody>
            {sortedTasks.length === 0 ? (
              <tr>
                <td colSpan="3" className="text-center p-8 text-slate-400 text-sm">
                  ...
                  <td className="p-4">
                    {editingId === task.id ? (
                      <input
                        ...
                        className="w-full px-3 py-1 border border-indigo-300 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                        autoFocus
                      />
                    ) : (
                      <span className="text-slate-700 font-medium text-sm">{task.name}</span>
                    )}
                  </td>
                  <td className="p-4">
                    <select
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-medium border cursor-pointer transition ${statusConfig[task.status].color}`}
                    >
